const projectsData = [
    {
        name: 'THE JIUTUBE',
        description: 'The JiuTube is a full stack React.js application that allows users to watch a live streaming Jiujitsu video player  through OBS and Twitch streaming technology while chatting about the matches on the Socket.IO real time chat that serves a chat history via api calls to the backend Flask server to query the SQL database for all previous comments.',
    ...
    {
        name: 'SURVEY DONKEY',
        description: 'SurveyDonkey is an interactive survey creation and participation site inspired by SurveyMonkey. Designed using PUG to render HTML and Bulma to bootstrap CSS on the front end. The backend server uses Express to handle Restful API requests and Sequelize to handle queries to the Postgres SQL Database.',
